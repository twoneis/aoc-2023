...
pub const PartTwo = struct {
    ...
    pub fn solve(self: Self, lines: [][]const u8) !T {
        ...
        for (0..lines.len) |k| {
            ...
            try self.shot_ray(&energized, lines, Cord{ .x = 0, .y = @as(i64, @intCast(k)) }, EAST);
        ...
        for (0..lines.len) |k| {
            ...
            try self.shot_ray(&energized, lines, Cord{ .x = @as(i64, @intCast(lines[0].len - 1)), .y = @as(i64, @intCast(k)) }, WEST);
        ...
        for (0..lines[0].len) |k| {
            ...
            try self.shot_ray(&energized, lines, Cord{ .x = @as(i64, @intCast(k)), .y = 0 }, SOUTH);
        ...
        for (0..lines[0].len) |k| {
            ...
            try self.shot_ray(&energized, lines, Cord{ .x = @as(i64, @intCast(k)), .y = @as(i64, @intCast(lines.len - 1)) }, NORTH);
